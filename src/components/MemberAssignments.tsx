            ...
            <Card key={assignment.id} className="hover:shadow-lg transition-all duration-200 border-l-4 border-l-orange-400">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <CardTitle className="text-xl text-orange-800 mb-2">
                      {assignment.templeDana?.temple?.name || 'Unknown Temple'}
                    </CardTitle>
                    <div className="flex items-center text-sm text-gray-600 mb-1">
                      <MapPin className="h-4 w-4 mr-2" />
                      {assignment.templeDana?.temple?.address || 'No address available'}
                    </div>
                  </div>
                  <div className="flex flex-col space-y-2">
                    ...
                    <Badge className={`${getTimeColor(assignment.templeDana?.dana?.time || '')} border font-medium`}>
                      ...
                      {assignment.templeDana?.dana?.time || 'Unknown Time'}
                    </Badge>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="bg-orange-50 p-4 rounded-lg border border-orange-100">
                  ...
                  <p className="text-lg font-medium text-gray-900 mb-1">{assignment.templeDana?.dana?.name || 'Unknown Dana'}</p>
                  <p className="text-sm text-gray-600 mb-2">{assignment.templeDana?.dana?.description || 'No description available'}</p>
                  <p className="text-xs text-gray-500">
                    Minimum Families Required: {assignment.templeDana?.minNumberOfFamilies || 0}
                  </p>
                </div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                  <h4 className="font-semibold text-blue-800 mb-3 flex items-center">
                    ...
                    Assigned Families ({assignment.templeDana?.assignments?.length || 0})
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {assignment.templeDana?.assignments?.map((familyAssignment) => (
                      <div key={familyAssignment.id} className="bg-white p-3 rounded-md border border-blue-200">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <p className="font-medium text-gray-900">{familyAssignment.family?.familyName || 'Unknown Family'}</p>
                            <div className="flex items-center text-xs text-gray-600 mt-1">
                              <Home className="h-3 w-3 mr-1" />
                              {familyAssignment.family?.address || 'No address'}
                            </div>
                            <div className="flex items-center text-xs text-gray-600 mt-1">
                              <Phone className="h-3 w-3 mr-1" />
                              {familyAssignment.family?.telephone || 'No phone'}
                            </div>
                          </div>
                          <Badge 
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 border-t border-gray-200">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <Button 
                      onClick={() => handleCallTemple(assignment.templeDana?.temple?.contactNumber || '')}
                      variant="outline"
                      size="sm"
                      className="w-full sm:w-auto border-blue-200 hover:bg-blue-50"
                      disabled={!assignment.templeDana?.temple?.contactNumber}
                    >
                      <Phone className="h-4 w-4 mr-2" />
                      Call Temple
                    </Button>
                    
                    {assignment.date && isTomorrow(assignment.date) && assignment.isConfirmed !== true && (
                      <Button 
                ...
                <div className="bg-gray-50 p-3 rounded-lg text-xs text-gray-600 border border-gray-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <p><span className="font-medium">Email:</span> {assignment.templeDana?.temple?.email || 'Not available'}</p>
                    <p><span className="font-medium">Website:</span> {assignment.templeDana?.temple?.website || 'Not available'}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
