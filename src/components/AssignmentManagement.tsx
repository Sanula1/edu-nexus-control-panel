...
export const AssignmentManagement = () => {
        ...
        return (
          <Card className="mb-6">
            ...
            <CardContent>
              <div className="flex gap-4 items-end">
                <div className="flex-1">
                  <Label htmlFor="selectedDate">Dana Date (YYYY-MM-DD)</Label>
                  <Input
                    id="selectedDate"
                    type="date"
                    value={selectedDate}
  ...
  const renderAssignmentCard = (assignment: TempleDanaAssignment) => {
    // Handle the updated API structure - temple is now consistently in templeDana.temple
    const temple = assignment.templeDana?.temple;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="font-semibold text-lg">{temple?.name || 'Unknown Temple'}</h3>
              </div>
              <p className="text-sm text-gray-600">{temple?.address || 'No address available'}</p>
              <p className="text-sm text-gray-500">{temple?.contactNumber || 'No contact number'}</p>
              <p className="text-sm text-gray-500">{temple?.email || 'No email available'}</p>
              {temple?.website && (
                <p className="text-sm text-gray-500">{temple.website}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="font-semibold text-lg">{assignment.templeDana?.dana?.name || 'Unknown Dana'}</h3>
              </div>
              <p className="text-sm text-gray-600">{assignment.templeDana?.dana?.description || 'No description'}</p>
              ...
              <p className="text-sm text-gray-500">
                Min Families: {assignment.templeDana?.minNumberOfFamilies || 0}
              </p>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="font-semibold text-lg">{assignment.family?.familyName || 'Unknown Family'}</h3>
              </div>
              <p className="text-sm text-gray-600">{assignment.family?.address || 'No address'}</p>
              <p className="text-sm text-gray-500">{assignment.family?.telephone || 'No phone'}</p>
              <div className="flex items-center space-x-2">
